""" 
python seq2seq-teacher-forcing.py --devid -1 --nhid 200 --nlayers 1 --epochs 15 --lr 1e-3 --wd 0 --bsz 64 --clip 5 --maxlen 20 --minfreq 5 --tf 0.50 --beam 100
...
def parse_args():
    ...
    parser.add_argument("--tf", type=float, default=0.50)
...
class Trainer:
    ...
    def train(self, num_epochs, model, lr = args.lr, weight_decay = args.wd, clip = args.clip):
        ...
        for epoch in tqdm(range(1, num_epochs + 1)):
            ...
            for batch in tqdm(self.train_iter):
                ...
                batch_loss = self.train_batch(batch, model, teacher_forcing_ratio = args.tf)
                batch_loss.backward()
    ...
    def train_batch(self, batch, model, teacher_forcing_ratio = 0):
        ...
        # Decider whether to use teacher forcing
        ...
        if use_teacher_forcing:
            ...
            for trg_word_idx in range(target_length-1):
                ...
                loss += F.cross_entropy(decoder_output.squeeze(0), batch.trg[trg_word_idx+1, :])
    ...
    def evaluate_kaggle(self, string, model, ngrams = 3, context = 1, top_k = args.beam):
        ...
        # Get top_k beams for 
        for trg_word_idx in range(1, ngrams+context-1):
...
if __name__ == '__main__':
    ...
    if USE_CUDA: model = model.cuda() # throws errors on GPU otherwise if not re-cuda'd
